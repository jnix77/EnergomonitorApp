...
class SensorDataRepository @Inject constructor(
    ...
    suspend fun fetchAndGroupSensorData(): Map<SensorTopic, List<SensorData>> {
        ...
        for (feed in feeds) {
            ...
            for (stream in streams) {
                // 3. For each stream, fetch the latest data point
                try {
        ...
        // 4. Group by Topic
        return allSensors.groupBy { determineTopic(it.title, it.unit) }
    }
    ...
    private fun determineTopic(title: String, unit: String): SensorTopic {
        ...
        return when {
            ...
            lowerUnit.contains("%") || lowerTitle.contains("humidity") -> SensorTopic.HUMIDITY
            lowerUnit.contains("w") || lowerTitle.contains("electric") || lowerTitle.contains("power") -> SensorTopic.ELECTRICITY
            lowerUnit.contains("m3") || lowerUnit.contains("l") || lowerTitle.contains("water") -> SensorTopic.WATER
            lowerTitle.contains("gas") -> SensorTopic.GAS
            else -> SensorTopic.UNKNOWN
        }
    }
}
